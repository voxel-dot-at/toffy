...
RasterSegment::RasterSegment(RasterPoint& start, RasterPoint& end) :
    	id(RID++),start(&start),end(&end),len(0),
	myGraph(0),
	pruned(false) {
...
RasterSegment::RasterSegment(const RasterSegment& o) : id(o.id), start(o.start), end(o.end),pts(o.pts),len(o.len)
    {
...
void RasterSegment::points(std::vector<RasterPoint*>& points, bool unVisitedOnly, bool addInlinePoints)
{
    if (unVisitedOnly) {
	if ( !start->visited) {
	    points.push_back(start);
    ...
    if (addInlinePoints) for (int i=0;i<pts.size();i++) {
	if (unVisitedOnly) {
	    if (! pts[i]->visited)
		points.push_back(pts[i]);
	    pts[i]->visited = true;
...
std::ostream& operator<<(std::ostream& o, const RasterPoint& p ) {
    o << "[ " << p.id << ": " << p.x << ", " << p.y << ", " << p.type << ", " << p.thick << " ]";
    return o;
}



void paintSegment(Mat& disMax, const RasterSegment& seg, Scalar color, int thickness ) {
	vector<Point*> pts;
	seg.points(pts);
