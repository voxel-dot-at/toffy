...
/****************************************SEG TRACER************************************************************/
SegmentTracer::SegmentTracer() : skeleton(0),thickness(0), 
				 keepAll(false) 
{
    debugging = false;
}
...
void SegmentTracer::trace(const Mat& skel)
{   
    pointList.resize(skel.cols);
}
...
void SegmentTracer::findPoints(int x, int y, std::vector<RasterPoint*>& hits, int maxDistance)
{
	for (int i=x-maxDistance;i<=x+maxDistance;i++) {
		if (i<0 || i>= pointList.size())
			continue;
...
void SegmentTracer::findPointsAt(int x, int y, std::vector<RasterPoint*>& hits, int distance)
{
...
void SegmentTracer::findPointsDiag(int x, int y, std::vector<RasterPoint*>& hits)
{
    int dist = 2;
...
RasterPoint* SegmentTracer::findPoint(int x, int y) {
	if (x<0 || (unsigned int)x > pointList.size())
		return 0;
