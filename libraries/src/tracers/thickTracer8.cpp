...
static bool hasUnvisitedNeighs(const cv::Mat& in, int x, int y,
                               int neighCount) {
  int a, b, c, d, e, f, g, h;
...
void ThickTracer8::trace(const Mat& mat) {
  ...
  if (debugging) {
    cout << "ThickTracer8 : Total : Found " << segments.size() << " Segments"
         << endl;
  }
  ...
  for (int i = 0; i < in.rows; i++) {
    ...
    for (int j = 0; j < in.cols; j++) {
      ...
      if (z == 1 || z == 0) {
        ...
        addPoint(
            new RasterPoint(j, i, endPoint, reaper->thickness.at<uchar>(i, j)));
        ends++;
      } else if (z == 2) {
  ...
  for (i = 0; i < (int)pts.size(); i++) {
    ...
    for (size_t j = 0; j < 8;
      ...
      if (!segments.contains(&pt, helper)) {
        ...
        if (dumpTracerDetails) cout << "C-C seg added " << *newSeg << endl;
      } else {
        if (dumpTracerDetails)
    ...
    while (hasNeighbours) {
      if (trc)
        cout << " START FOLLOW " << pt << " NEIGHU " << hex << count << dec
             << endl;
...
void ThickTracer8::followLine(cv::Mat& in, RasterPoint& start,
  ...
  while (count == 1) {
    ...
    if (false && dumpTracerDetails)
      cout << "  trc nxt: " << p << " " << count << " b=" << hex << (int)bits
           << dec << " i=" << i << endl;
    ...
    if (keepAll) {
      ...
      if (partLen >= 30) {
        partLen = 0;
        seg.pts.push_back(
            new RasterPoint(x, y, cornerPoint, thickness->at<char>(y, x)));
      }
    }
    if (false && dumpTracerDetails)
      cout << "  trc: " << p << " " << count << " b=" << hex << (int)bits << dec
           << endl;
    ...
    // Find next Point
    count = getNeighs(in, p.x, p.y, dir, neigh, &bits);  // Check for next pixel

    if (dumpTracerDetails)
      cout << "  trc nxt2: " << p << " " << count << " b=" << hex << (int)bits
           << dec << " count=" << count << endl;
  }
  // end of segment...
  if (dumpTracerDetails)
  ...
  if (helper) {
    if (helper->type == endPoint) {
      if (trc)
        cout << " helper is endpoint " << endl;  // helper->visited = true;
    } else if (trc)
