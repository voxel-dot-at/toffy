...
class RasterPoint : public cv::Point_<int>
{
   public:
    RasterPoint()
        : cv::Point_<int>(-1, -1),
    ...
    RasterPoint(const cv::Point2i& p)
        : cv::Point_<int>(p.x, p.y),
          id(-1),
          type(inLine),
          thick(0),
          count(0),
    ...
    int id;
    rasterPointType
        type;  ///< type (referring to segments/graph - endpoint, crossingpint,... )
...
class RasterSegment
{
   ...
   public:
     ...
     */
    const RasterPoint* nextInteresting(const RasterPoint* oneEnd,
                                       int dist = 1) const
    {
        if (oneEnd == start) {
    ...
    void nearest(RasterSegment* checkMe, RasterPoint*& a, RasterPoint*& b,
                 int& dStart, int& dEnd)
    {
        int ss = manhattan(start, checkMe->start);
        int se = manhattan(start, checkMe->end);
