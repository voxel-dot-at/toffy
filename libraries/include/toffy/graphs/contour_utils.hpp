 ...
 */
static int findNearestContourPt(const std::vector<cv::Point>& contour,
                                cv::Point2f p, float& dist)
{
    int minIdx = -1;
 ...
 * </pre>
 */
static inline void getAngle(const cv::Point& a, const cv::Point& b, int& angle,
                            float& dist)
{
    int dx = b.x - a.x;
    int dy = b.y - a.y;
 ...
 */
static bool computeTripleAngle(const std::vector<cv::Point>& contour, int idx,
                               int distance, float& cosAngle)
{
    int idxA = idx - distance;
    if (idxA < 0) idxA += contour.size();
    int idxC = idx + distance;
    if (idxC >= (int)contour.size()) idxC -= contour.size();
...
static void initCurv(cv::Mat& m, const std::vector<cv::Point>& contour)
{
    ...
    // zero line
    line(m, cv::Point(0, 100), cv::Point(contour.size() - 1, 100),
 ...
 */
static void printCurv(cv::Mat& m, cv::Scalar color,
                      const std::vector<cv::Point>& contour, int distance)
{
    float cos;
