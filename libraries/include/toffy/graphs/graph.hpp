...
{
   public:
    TouchInfo(Karterl* k = 0, cv::Point* p = 0)
        : karterl(k), point(p), numPoints(0), isArrow(false)
    {
    }

    TouchInfo(const TouchInfo& ti)
        : karterl(ti.karterl),
          point(ti.point),
          numPoints(ti.numPoints),
          isArrow(ti.isArrow)
    {
    }
...
class Graph
{
   public:
    ...
    Graph(std::vector<RasterSegment*>& segs)
        : segs(segs),
          touchesRoi(false),
          leavesRoi(false),
          isLoop(false),
    ...
    virtual bool isNear(RasterSegment* checkMe, int fuzz = 2,
                        int minFuzz = 5) const
    {
        //std::cout << "ck " << segs.size() << " ";
        for (unsigned int i = 0; i < segs.size(); i++) {
            if (checkMe->isNear(segs[i], fuzz, minFuzz)) return true;
        }
        return false;
    }
    ...
    virtual void points(std::vector<RasterPoint*>& points,
                        bool unVisitedOnly = false,
                        bool addInlinePoints = false) const
    {
        for (unsigned int i = 0; i < segs.size(); i++) {
    ...
    virtual void unPrunedPoints(std::vector<RasterPoint*>& points,
                                bool unVisitedOnly = false) const
    {
        for (unsigned int i = 0; i < segs.size(); i++) {
            if (!segs[i]->pruned) segs[i]->points(points, unVisitedOnly);
        }
    }
