 ...
 */
static RasterPoint* findFarthestPoint(const cv::Point2i& ref,
                                      const std::vector<RasterPoint*>& points)
{
    ...
    */
    ...
    for (size_t i = 0; i < points.size(); i++) {
        if (points[i]->type != endPoint) continue;
        tmp = points[i];
        break;
    }
...
template <typename T>
T* findNearestPoint(cv::Point2i& ref, const std::vector<T*>& points,
                    bool sorted = true, bool onlyEndpoints = true)
{
    T* min = points[0];
    float dist = 1000.;
    float last = 1000.;
    ...
    for (size_t i = 0; i < points.size(); i++) {
        ...
        if (onlyEndpoints && rp.type != endPoint) continue;
 ...
 */
static std::vector<const RasterPoint*> getPtsSorted(
    const RasterPoint* from, const RasterPoint* /*to*/,
    const std::vector<const RasterSegment*>& list, int maxPoints = -1)
{
...
static void plotSegs(cv::Mat& img, const std::vector<RasterSegment*>& segs,
                     cv::Scalar color)
{
...
static std::vector<const RasterSegment*> getCon(const RasterPoint* from,
                                                const RasterPoint* to)
{
    ...
    if (dbgTrace) {
        ...
        cout << "C f " << *from << " s=" << from->segs.size()
             << " v=" << from->visited;
        if (from == to) cout << "***";
        cout << endl;
    }
    ...
    for (size_t i = 0; i < from->segs.size(); i++) {
        ...
        if (next == to ||
            (to && (to->type == inLine &&
                    find(from->segs[i]->pts.begin(), from->segs[i]->pts.end(),
                         to) != from->segs[i]->pts.end()))) {
