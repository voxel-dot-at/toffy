...
BlobsDetector::BlobsDetector(): Filter(BlobsDetector::id_name), in_img("fg"), in_ampl("ampl"),
    out_blobs("blobs"), _minSize(40), amplFilter(false), minAmpl(220),
    refineBlobs(false), morpho(false), sharpenEdges(false)
{
    _filter_counter++;
}
...
void BlobsDetector::updateConfig(const boost::property_tree::ptree &pt) {
    ...
    _params.minDistBetweenBlobs = pt.get<float>("options.minDistBetweenBlobs", 10);
    ...
    _params.filterByCircularity = pt.get<bool>("options.filterByCircularity", false);
    ...
    _params.maxInertiaRatio = pt.get<float>("options.maxInertiaRatio", (float)1e37);
...
void BlobsDetector::findBlobs(cv::Mat& img, cv::Mat& ampl, int fc, std::vector<DetectedObject*>& detObj)
{
    ...
    drawKeypoints(m, keyImg, result, Scalar::all(-1), DrawMatchesFlags::DEFAULT & DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
    imshow("rest", result);
    //waitKey();
