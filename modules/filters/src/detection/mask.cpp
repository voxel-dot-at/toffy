...
Mask::Mask()
    : Filter(Mask::id_name, _filter_counter),
      _in_depth("depth"),
      _in_ampl("ampl"),
      _out_mask("mask")
{
...
void Mask::updateConfig(const boost::property_tree::ptree &pt)
{
    ...
    if (ocvo.is_initialized()) {
        if (toffy::commons::checkOCVNone(*ocvo)) {
            ...
            if (!_cameraMatrix) _cameraMatrix.reset(new cv::Mat());
...
bool Mask::filter(const Frame &in, Frame &out)
{
    ...
    if (_creation) {
        ...
        cv::calibrationMatrixValues(*_cameraMatrix, depth->size(),
                                    _apertureWidth, _apertureHeight, fovx, fovy,
        ...
        */
        ...
        fillMaskGaps();
        if (dbg) {
            cv::namedWindow(id_name + "[4] new_mask insider",
                            cv::WINDOW_NORMAL);
        ...
        *depth_mask /= 2.f;
        ...
        cv::morphologyEx(*depth_mask, *depth_mask, cv::MORPH_CLOSE,
...
static inline double deg2rad(float degrees) { return (degrees * M_PI) / 180.0; }
...
void Mask::groundProjection2()
{
    ...
    for (int y = 0; y < img3d->rows; ++y) {
        for (int x = 0; x < img3d->cols; ++x) {
            //cout << "(" << x << "," << y << ")" << endl;
            // Avoing Z NaN values
            if (channel[2].at<float>(y, x) !=
                channel[2].at<float>(
                    y,
                    x))  //test.at<float>(y,x) = numeric_limits<double>::quiet_NaN ();
                continue;
            //TODO PARAMETERS!!
            if (channel[2].at<float>(y, x) > _dis ||
                channel[2].at<float>(y, x) < _dis * 0.05)
                continue;
            // Go for cm
            ...
            // beyond the sizes.
            if (valX < 0 || valY < 0 || valX >= (int)maxSizeX ||
                valY >= (int)maxSizeY) {
                //cout << "(" << valX << "," << valY << ")" << endl;
                continue;
            }
...
void Mask::projectBack()
{
    ...
    for (int y = 0; y < fground->rows; ++y) {
        for (int x = 0; x < fground->cols; ++x) {
            ...
            // Saving found pixel
            if (iValX >= 0 && iValX < new_mask->cols && iValY >= 0 &&
                iValY < new_mask->rows)
...
void Mask::morfExMask(cv::Mat &mask)
{
    ...
    structuringElement = cv::getStructuringElement(
        cv::MORPH_ELLIPSE, cv::Size(2 * morph_size + 1, 2 * morph_size + 1),
        cv::Point(morph_size, morph_size));
    cv::morphologyEx(antiMask, antiMask, cv::MORPH_OPEN, structuringElement);
...
void Mask::fillMaskGaps()
{
    ...
    cv::findContours(new_mask->clone(), contours, hierarchy, cv::RETR_CCOMP,
...
void Mask::generate3D()
{
    ...
    for (int y = 0; y < static_cast<int>(depth->rows); ++y) {
         ...
         */
            //Set 3D points
            dptr[0] = (static_cast<float>(x) - center_x) * depthValue *
                      fl_x_reciprocal;  //X
            dptr[1] = (static_cast<float>(y) - center_y) * depthValue *
                      fl_y_reciprocal;  //Y
            dptr[2] = depthValue;       //Z
        }
    }
