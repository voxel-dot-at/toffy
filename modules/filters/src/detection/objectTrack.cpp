...
ObjectTrack::ObjectTrack(): Filter(ObjectTrack::id_name,_filter_counter),
			    _in_img("img"), _in_vec("objects"), _out_vec("dect"), cnt(0), numObjects(0), updateScript("updateCount.sh")
{
    _filter_counter++;
...
bool ObjectTrack::filter(const Frame& in, Frame& out) {
    ...
    try {
        detObjs = boost::any_cast<boost::shared_ptr<std::vector<detection::DetectedObject*> > >(in.getData(_in_vec));
    ...
    BOOST_LOG_TRIVIAL(debug) << "Got _in_vec(" << _in_vec << "): " << detObjs->size();
    ...
    while ( objDetIter != detObjs->end() ) {
        ...
        double dis = cv::norm(obj->massCenter3D - obj->record->front()->massCenter3D );
        ...
        cv::arrowedLine(color, obj->record->back()->massCenter, obj->massCenter, CV_RGB(255, 255, 0));

        cv::line(color,cv::Point(z1.x,z1.height),z1.br(),CV_RGB(255, 0, 0),1);

        cv::putText	(color, boost::str(boost::format("%.4f") % speed) + " m/s", cv::Point(10,20),
