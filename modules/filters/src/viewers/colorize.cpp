...
Colorize::Colorize(): Filter(Colorize::id_name,filter_counter),
		      scale(1.f), max(1.5), min(0.01), in_img("depth"),out_img("colored"), colormap("jet")
{
    filter_counter++;
}
...
bool Colorize::filter(const Frame &in, Frame& out)
{
	...
	"missing image " << in_img <<
	", filter  " << id() <<" not applied.";
      return false;
    }

    if (! out.hasKey(out_img)) {
	colored.reset(new Mat(Size(img->rows*scale, img->cols*scale), CV_8UC3)); 
    } else {
    ...
    diff = boost::posix_time::microsec_clock::local_time() - start;
    BOOST_LOG_TRIVIAL(debug) << "Colorize gray2rgb: " << diff.total_microseconds();
    ...
    if( scale != 1.f ) {
      ...
      diff = boost::posix_time::microsec_clock::local_time() - start;
      BOOST_LOG_TRIVIAL(debug) << "Colorize resize: " << diff.total_microseconds();
