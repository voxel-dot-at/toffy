...
void ExportCSV::updateConfig(const boost::property_tree::ptree &pt)
{
    ...
    LOGD << __FUNCTION__ << " fp " << _filePattern;
    LOGD << __FUNCTION__ << " fc " << _fc;
}
...
static void saveMatCSV(const std::string &fileName, const Mat &mat,
                       bool skipZeroes);
...
bool ExportCSV::filter(const Frame &in, Frame &)
{
    ...
    try {
        ...
        LOG(warning) << "Could not cast input " << _in << ", filter  " << id()
                     << " not applied.";
        return false;
    }
    ...
    if (_seq) {
        ...
        if (_fc.length()) {
            ...
            snprintf(path, _filePattern.length() + 64, _filePattern.c_str(),
                     fc);
        } else {
            LOG(warning) << "no suitable file pattern found";
            snprintf(path, _filePattern.length() + 64, "%s",
                     _filePattern.c_str());  // no arg?!
        }
    }
                       ...
                       bool /*skipZeroes*/)
{
    ...
    switch (mat.type()) {
        ...
        case CV_64FC1:
            ...
            break;
        default:
            LOGW << "no suitable mat data conversion for " << mat.type() << " "
                 << " type " << typeToString(mat.type()) << " depth "
