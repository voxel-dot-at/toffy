...
Transform::Transform()
    : Filter(Transform::id_name, _filter_counter),
      _in_cloud("cloud"),
      _out_cloud(_in_cloud)
{
    _filter_counter++;
}

bool Transform::filter(const Frame &in, Frame &out)
{
    LOGD << __FUNCTION__ << " " << id();
    pcl::RangeImagePlanar::Ptr planar;
    try {
        planar =
            std::any_cast<pcl::RangeImagePlanar::Ptr>(in.getData(_in_cloud));
    } catch (const std::bad_any_cast &) {
        LOGW << "Could not cast input " << _in_cloud << ", filter  " << id()
             << " not applied.";
        return false;
    }
    Eigen::Affine3f tr;
    ...
    for (size_t i = 0; i < _actions.size(); i++) {
        if (_operations[i] == Transform::rotation) {
            ...
            Eigen::Translation3f t(_actions[i][0], _actions[i][1],
                                   _actions[i][2]);
            tr = t * tr;
        } else if (_operations[i] == Transform::scaling) {
            //Eigen::Affine3f m = ;
            tr = Eigen::Scaling((float)_actions[i][0], (float)_actions[i][1],
                                (float)_actions[i][2]) *
                 tr;
        } else
            LOGW << "Operation unknown.";
    }
...
void Transform::updateConfig(const boost::property_tree::ptree &pt)
{
    ...
    try {
        ...
        LOGD << "Transform values are wrong. Ex:" << ex.what();
    }
