...
bool SampleConsensus::runEuclidean(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
  ...
  LOG(info) << "*** EUCI *** dis " << distanceThresh 
    << " size " << cloud->size()
    << " min " << minSize
    << " max " << maxSize
    ;
  ...
  LOG(info) << "*** EUCI *** L " << __LINE__ << " l " << cluster_indices.size();
...
bool SampleConsensus::getInputPoints(
    const Frame& in, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
  ...
  try {
    ...
    LOG(warning) << "Could not cast input - trying to convert from OCV cloud.";
    ...
    LOG(info) << "copy from matPtr{____}" << __LINE__;
      cloud.reset(new pcl::PointCloud<pcl::PointXYZ>(/*width=*/img3d->cols,
                                                     /*height=*/img3d->rows));
    LOG(info) << "copy from matPtr{____}" << __LINE__;
      float* dptr;
    ...
    LOG(info) << "copy from matPtr{____} >>" << __LINE__ << " " << cloud->size();
    } catch (const boost::bad_any_cast&) {
...
void SampleConsensus::updateConfig(const boost::property_tree::ptree& pt) {
  ...
  LOG(debug) << "set up for " << sampleTypeNames[type] << " iters " << maxIters
             << " thresh " << threshold << " outputPC2? " << outputPC2;
}
