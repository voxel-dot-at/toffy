...
GroundProjection::GroundProjection(): Filter(GroundProjection::id_name,_filter_counter),
	_in_cloud("cloud"), _in_img("ground"), _out_img("ground"), _max_y(0), _max_x(0),
	_scale(100), _fovx(90.), _fovy(67.5), _dis(0.0), _projBack(false) {
    _filter_counter++;
...
bool GroundProjection::filter(const Frame &in, Frame& out) {
    ...
    double maxSizeX , maxSizeY,
	    fl_x_reciprocal, fl_y_reciprocal;
    Point2d center;
	...
	calibrationMatrixValues(_cameraMatrix, imgSize,
				apertureWidth, apertureHeight,
				_fovx, _fovy, noV, center, noV);
    }
	...
	try {
	    cloud = boost::any_cast<pcl::PCLPointCloud2Ptr>(in.getData(_in_cloud));
	} catch(const boost::bad_any_cast &) {
	    BOOST_LOG_TRIVIAL(warning) << "Could not cast input " << _in_cloud <<
	...
	if (_dis > 0.0) {
		...
		BOOST_LOG_TRIVIAL(warning) << "Wrong max values, x=" << _max_x
					   << ", y=" << _max_y << ", filter  "
					   << id() << " not applied.";
		return false;
	    }
	...
	cout << "Sizes maxSizeX:" << maxSizeX << " maxSizeY: " << maxSizeY << endl;
	...
	try {
		...
		", filter  " << id();
	    proj2d.reset(new Mat(maxSizeY, maxSizeX,
				 CV_32F,
				 numeric_limits<float>::quiet_NaN ()));
	    out.addData(_out_img,proj2d);
	}

	BOOST_LOG_TRIVIAL(debug) << "Sizes: " << proj2d->size() << " ==  " << Size(maxSizeX,maxSizeY);
	...
	pcl::PointCloud<pcl::PointWithRange>::Ptr cloudXYZ( new pcl::PointCloud<pcl::PointWithRange>);
	...
	*pback=0.0;
	...
	for (int y=0; y < fground->rows; ++y) {
		...
		cout << "fground->at<float>(y,x) " << fground->at<float>(y,x) << endl;
		//maxSizeX = (tan(pcl::deg2rad(_fovx)/2)*fground->at<float>(y,x)*2)*_scale;
		//maxSizeY = (tan(pcl::deg2rad(_fovy)/2)*fground->at<float>(y,x)*2)*_scale;
		//cout << "maxSizeX " << maxSizeX << endl;
		//cout << "maxSizeY " << maxSizeY << endl;
		...
		//Projecting back to image 2D coordinates
		int iValX = fValX/(fground->at<float>(y,x)*fl_x_reciprocal);
		int iValY = fValY/(fground->at<float>(y,x)*fl_y_reciprocal);
		...
		// Saving found pixel
		if ( iValX >= 0 && iValX < pback->cols
		     && iValY >= 0 && iValY < pback->rows )
...
/*
void GroundProjection::projectBack(const cv::Mat &mask, double maxSizeX , double maxSizeY) {
	...
	for (int x=0; x < proj2d.cols; ++x) {
	    ...
	    //Projecting back to image 2D coordinates
	    //int iValX = (fValX/(proj2d.at<float>(y,x)*fl_x_reciprocal)) + center_x;
	    //int iValY = (fValY/(proj2d.at<float>(y,x)*fl_y_reciprocal)) + center_y;
	    // Saving found pixel
