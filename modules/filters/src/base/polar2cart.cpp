...
Polar2Cart::Polar2Cart(): Filter(Polar2Cart::id_name,_filter_counter),
    _fovx(-1), _fovy(-1),_in_img("img"), _out_img(_in_img)
{
    _filter_counter++;
}
...
bool Polar2Cart::filter(const Frame &in, Frame& out) {
	...
	BOOST_LOG_TRIVIAL(error) << "Could not cast input " <<
				    _in_img << ", filter  " <<
				    id() <<" not applied.";
	return false;
    }
	...
	double noV, apertureWidth = (45/1000)*img->size().width, apertureHeight = (45/1000)*img->size().height;
	Point2d np;
	calibrationMatrixValues(_cameraMatrix, img->size(), apertureWidth, apertureHeight, _fovx, _fovy, noV, np, noV);
    }
	...
	for(int j=0; j < new_img.cols; j++) {
	    if (new_img.at<float>(i,j) < 65. && new_img.at<float>(i,j) > 0. &&  new_img.at<float>(i,j) == new_img.at<float>(i,j)) {
