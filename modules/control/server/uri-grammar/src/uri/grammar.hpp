...
struct ipv4_grammar :
	boost::spirit::qi::grammar<Iterator, boost::uint32_t()> {
	...
	ipv4address
		...
		dec_octet
		=   boost::spirit::qi::uint_parser<boost::uint8_t, 10, 1, 3>()
		;
    }
		...
		// properly.
		//
		boost::spirit::qi::rule<Iterator, boost::uint32_t()> ipv4address,
		dec_octet;
    };

		template <typename Iterator>
		struct ipv6_grammar :
		boost::spirit::qi::grammar<Iterator, boost::spirit::qi::locals<unsigned>, boost::array<boost::uint16_t, 8>()> {
		...
		ipv6address
			...
			boost::array<boost::uint16_t, 8>()>
			ipv6address;
			boost::spirit::qi::rule<Iterator, std::pair<boost::uint16_t, boost::uint16_t>()> ls32;
			boost::spirit::qi::uint_parser<boost::uint16_t, 16, 1, 4> h16;
