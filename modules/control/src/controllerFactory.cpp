...
boost::container::flat_map<std::string, FilterController* > ControllerFactory::_controllers;
ControllerFactory::ControllerFactory(){}

ControllerFactory * toffy::control::ControllerFactory::getInstance() {
  if(_uniqueFactory == NULL)
    _uniqueFactory = new ControllerFactory();
...
FilterController * ControllerFactory::getController(Filter *f) {
    ...
    try {
	    ...
	    BOOST_LOG_TRIVIAL(info) << f->id() << " not found. Instantiating default controller";
    }
...
FilterController * ControllerFactory::createController(Filter *f) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "ControllerFactory::createController() for f->name(), id(): " << f->name() << "\t" << f->id();

    _controllers.insert(std::pair<std::string, FilterController*>(f->id(),fc));

    return fc;
}

int ControllerFactory::getControllersByType(const std::string& type,
				     std::vector<FilterController *> &vec)
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__;
    for (boost::container::flat_map<std::string, FilterController* >::iterator it=_controllers.begin();
...
boost::container::flat_map<std::string, CreateFilterControllerFn> ControllerFactory::_creators;

void ControllerFactory::registerCreator( std::string name, CreateFilterControllerFn fn)
{ 
    _creators.insert( std::pair<std::string, CreateFilterControllerFn>(name, fn) );
}
