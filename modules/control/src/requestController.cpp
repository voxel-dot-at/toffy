...
int Controller::processRequest(http::server::connection_ptr con,
	...
	name = name.substr(0,pos);
      ...
      BOOST_LOG_TRIVIAL(debug) << "get image " << name
			       << " " << action._items[2];
      
      //      _thread = boost::thread( boost::bind(&Controller::loopImage, this, con,req,rep, name));

	return true;
#if defined(ENABLE_PCL)
    } else if (name == "cloud") {
	BOOST_LOG_TRIVIAL(debug) << "cloud";
	_thread = boost::thread( boost::bind(&Controller::loopCloud, this, con,req,rep));

	return true;
#endif
    } else if (name == "actions") {

	_thread = boost::thread( boost::bind(&Controller::loopAction, this, con,req,rep));
	...
	if (action._items[2] == "bta" || action._items[2] == "sensor2d") {
	    ...
	    BOOST_LOG_TRIVIAL(debug) << "action._items[2]: " << action._items[2];
	...
	boost::property_tree::ptree::const_assoc_iterator it = jsonValue.find("file");
	...
	//We want the controller factory finding the filters and instantiating the controller if exist
	Filter *f = _c->baseFilterBank->getFilter(name);
	//BOOST_LOG_TRIVIAL(debug) << f;
	//BOOST_LOG_TRIVIAL(debug) << _bankController.getFilter(action._items[1]);
	if (f == NULL) {
	    jOutput.add("status","failed");
    ...
    #endif
	//} else {
	    FilterController *fc = ControllerFactory::getInstance()->getController(f);
...
void Controller::collectFilters(std::string &log, toffy::FilterBank* bank, boost::property_tree::ptree& jsonList)
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__;
	...
	if (f->type() == "filterBank" ||
		f->type() == "parallelFilter" ) {
	    const FilterBank* fb = static_cast<const FilterBank*>(f);

	    //jsonList.push_back(std::make_pair("", bank->getFilter(i)->name()));
...
void Controller::collectFilterTree(std::string &log, toffy::FilterBank* bank, boost::property_tree::ptree& jsonList)
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__;
    for (size_t i=0; i < bank->size() ; i++) {
	const Filter* f = bank->getFilter(i);
...
void Controller::loopImage(http::server::connection_ptr con,
			   const http::server::request &req, http::server::reply &rep, std::string name)
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__;
    //http::server::reply rep;
    ...
    rep.headers[1].value = "multipart/x-mixed-replace;boundary=--BoundaryString";
    ...
    while(con->socket().is_open()) {
      BOOST_LOG_TRIVIAL(debug) << "IMAGE THREAD " << con->socket().available();

      if (_c->f.hasKey(name)) {
	try {
        body = _c->f.getMatPtr(name)->clone();
	} catch(std::exception& e) {
	  BOOST_LOG_TRIVIAL(debug) << "Todo: fix race condition " << e.what();
	...
	BOOST_LOG_TRIVIAL(debug) << "img loop had " << body.type() << " " << show.size() ;
	...
	putText(show, name + " not found...", cv::Point(12,12), FONT_HERSHEY_PLAIN, 1, cv::Scalar(255,255,255) );
      }
      ...
      const std::string mpartHeader =  "--BoundaryString\r\n"
...
bool Controller::loadConfig(std::string config,
				std::string &log)
{
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__;
	...
	if (_c->loadConfig(pxml) < 1) {
		...
		#ifdef MSVC
		toffy::commons::plugins::initUI_t initUI = (toffy::commons::plugins::initUI_t)GetProcAddress(static_cast<HINSTANCE>(loads[i]), "initUI");
		if (!initUI) {
		    BOOST_LOG_TRIVIAL(warning) << "Could not load plug-in filters interface.";
		#else
		toffy::commons::plugins::initUI_t initUI = (toffy::commons::plugins::initUI_t) dlsym(loads[i], "initUI");
		const char *dlsym_error = dlerror();
		if (dlsym_error) {
		    BOOST_LOG_TRIVIAL(info) << "Could not load plug-in filters interface: "
					       << std::endl
					       << dlsym_error;
		#endif
		} else {
		    // use it to do the calculation
		   BOOST_LOG_TRIVIAL(info) << "Loaded plug-in filters interface";
		   // use it to do the calculation
	       ...
	       const http::server::request &req, http::server::reply &rep)
{
    ...
    pcl::PCLPointCloud2 pc2 = *boost::any_cast<pcl::PCLPointCloud2Ptr>(_c->f.getData("cloud"));
	...
	for(size_t i = 0; i < pc2.fields.size(); i++) {
	    ...
	    jOutput.add("cloud.fields."+cnt+".datatype", pc2.fields[i].datatype);
	}
	...
	typedef base64_from_binary<transform_width<iterator_type, 6, 8> > base64_enc;
	std::copy(base64_enc(pc2.data.begin()), base64_enc(pc2.data.end()), ostream_iterator<pcl::uint8_t>(ss));
	       ...
	       const http::server::request &req, http::server::reply &rep)
{
    ...
    boost::shared_ptr<boost::circular_buffer<boost::shared_ptr<toffy::Actions::Action> > >actionsBuffer;
    try {
	actionsBuffer = boost::any_cast<boost::shared_ptr<boost::circular_buffer<boost::shared_ptr<toffy::Actions::Action> > > >(_c->f.getData("actions"));
